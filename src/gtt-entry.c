...
static void gnome_entry_set_history_id(GnomeEntry *gentry, const gchar *history_id);

static void gnome_entry_prepend_history(GnomeEntry *gentry, gboolean save, const gchar *text);
...
static gboolean gnome_entry_mnemonic_activate(GtkWidget *widget, gboolean group_cycling)
{
    ...
    if (!GTK_WIDGET_IS_SENSITIVE(GTK_COMBO(entry)->entry))
        ...
        g_signal_emit_by_name(
            GTK_COMBO(entry)->entry, "mnemonic_activate", group_cycling, &handled
        );
...
static void gnome_entry_class_init(GnomeEntryClass *class)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_string(
            "history_id", _("History ID"), _("History ID"), NULL, G_PARAM_READWRITE
        )
    );
    g_object_class_install_property(
        gobject_class, PROP_GTK_ENTRY,
        g_param_spec_object(
            "gtk_entry", _("GTK entry"), _("The GTK entry"), GTK_TYPE_ENTRY, G_PARAM_READABLE
        )
    );
}
