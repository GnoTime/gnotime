...
static PropDlg *prop_dialog_new(void)
{
    ...
    GtkWidget *tmp_outer_entry = NULL;
    GtkWidget *tmp_inner_entry = NULL;

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "project_title", "entry7", "title box",
        _("A title to assign to this project")
    );
    dlg->title = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(title_table), tmp_outer_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "project_description", "entry9", "desc box",
        _("a short description that will be printed on the invoice.")
    );
    dlg->desc = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(title_table), tmp_outer_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );
    ...
    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "regular_rate", "combo regular", "regular box",
        _("The dollars per hour normally charged for this project.")
    );
    dlg->regular = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(rate_table), tmp_outer_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "overtime_rate", "overtime combo", "overtime box",
        _("The dollars per hour charged for overtime work on this project.")
    );
    dlg->overtime = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(rate_table), tmp_outer_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "overover_rate", "overover combo", "overover box",
        _("The over-overtime rate (overtime on Sundays, etc.)")
    );
    dlg->overover = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(rate_table), tmp_outer_entry, 1, 2, 2, 3, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "flat_fee", "flatfee combo", "flatfee box",
        _("If this project is billed for one price no matter how long it takes, enter the fee "
          "here.")
    );
    dlg->flatfee = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(rate_table), tmp_outer_entry, 1, 2, 3, 4, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    GtkWidget *const interval_table = glade_xml_get_widget(gtxml, "interval table");

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "min_interval", "entry22", "minimum box",
        _("Intervals smaller than this will be discarded")
    );
    dlg->minimum = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(interval_table), tmp_outer_entry, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "merge_interval", "entry23", "interval box",
        _("Time below which an interval is merged with its neighbors")
    );
    dlg->interval = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(interval_table), tmp_outer_entry, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );

    gtt_create_gnome_entry(
        &tmp_outer_entry, &tmp_inner_entry, "merge_gap", "entry24", "gap box",
        _("If the gap between intervals is smaller than this, the intervals will be merged "
          "together.")
    );
    dlg->gap = GTK_ENTRY(TAGGED_WIDGET(tmp_inner_entry));
    gtk_table_attach(
        GTK_TABLE(interval_table), tmp_outer_entry, 1, 2, 2, 3, GTK_EXPAND | GTK_FILL, 0, 0, 0
    );
