...
yearday_to_centuryday (int yday, int year)
{
  ...
  cd -= (year - 1)
        / 100; /* year that are multiple of 100 are not leap years */
  return cd;
}
...
count_days (DayArray *da, GttProject *proj, gboolean include_subprojects)
{
  ...
  da->start_cday
      = yearday_to_centuryday (da->start_tm.tm_yday, da->start_tm.tm_year);
}
...
int
gtt_project_foreach_interval (GttProject *proj, GttIntervalCB cb,
                              gpointer data)
{
  int rc = 1;
  GList *tnode, *inode;
...
int
gtt_project_foreach_subproject_interval (GttProject *proj, GttIntervalCB cb,
                                         gpointer data)
{
  int rc = 0;
  GList *pnode;
...
static int
cmp_earliest (GttInterval *ivl, gpointer data)
{
...
static int
cmp_latest (GttInterval *ivl, gpointer data)
{
  time_t *latest = data;
