...
static void gtt_projects_tree_row_expand_collapse_callback (GtkTreeView *view,
                                                            GtkTreeIter *iter,
                                                            GtkTreePath *path,
                                                            gpointer data);

static void gtt_projects_tree_model_row_changed_callback (GtkTreeModel *model,
                                                          GtkTreePath *path,
                                                          GtkTreeIter *iter,
                                                          gpointer data);
...
gtt_projects_tree_create_model (GttProjectsTree *gpt)
{
  ...
  tree_model
      = gtk_tree_store_new (NCOLS, G_TYPE_STRING, /* TIME_EVER_COLUMN */
...
gtt_projects_tree_class_init (GttProjectsTreeClass *klass)
{
  ...
  projects_tree_signals[COLUMNS_SETUP_DONE]
      = g_signal_new ("columns_setup_done", G_TYPE_FROM_CLASS (object_class),
                      G_SIGNAL_RUN_LAST, 0, NULL, NULL,
                      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void
gtt_projects_tree_set_time_value (GttProjectsTree *gpt,
                                  GtkTreeStore *tree_model, GtkTreeIter *iter,
  ...
  if (value == 0)
  ...
  else
    {
      if (priv->show_seconds)
        {
          sprintf (buff, "%02d:%02d:%02d", value / 3600, (value / 60) % 60,
                   value % 60);
        }
      else
        {
...
static void
gtt_projects_tree_set_date_value (GttProjectsTree *gpt,
                                  GtkTreeStore *tree_model, GtkTreeIter *iter,
                                  gint column, time_t value)
{
  gchar buff[100];
  memset (buff, 0, 100);
  if (value > -1)
...
static void
gtt_projects_tree_set_project_times (GttProjectsTree *gpt,
                                     GtkTreeStore *tree_model, GttProject *prj,
                                     GtkTreeIter *iter)
{
  GtkTreePath *path
      = gtk_tree_model_get_path (GTK_TREE_MODEL (tree_model), iter);

  if (gtk_tree_view_row_expanded (GTK_TREE_VIEW (gpt), path))
    {
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_EVER_COLUMN,
                                        gtt_project_get_secs_ever (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_YEAR_COLUMN,
                                        gtt_project_get_secs_year (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_MONTH_COLUMN,
                                        gtt_project_get_secs_month (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_WEEK_COLUMN,
                                        gtt_project_get_secs_week (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_LASTWEEK_COLUMN,
                                        gtt_project_get_secs_lastweek (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_YESTERDAY_COLUMN,
                                        gtt_project_get_secs_yesterday (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_TODAY_COLUMN,
                                        gtt_project_get_secs_day (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_TASK_COLUMN,
                                        gtt_project_get_secs_current (prj));
    }
  else
    {
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
      ...
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_YEAR_COLUMN,
                                        gtt_project_total_secs_year (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_MONTH_COLUMN,
                                        gtt_project_total_secs_month (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_WEEK_COLUMN,
                                        gtt_project_total_secs_week (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_LASTWEEK_COLUMN,
                                        gtt_project_total_secs_lastweek (prj));
      gtt_projects_tree_set_time_value (
          gpt, tree_model, iter, TIME_YESTERDAY_COLUMN,
          gtt_project_total_secs_yesterday (prj));
      gtt_projects_tree_set_time_value (gpt, tree_model, iter,
                                        TIME_TODAY_COLUMN,
...
static void
gtt_projects_tree_set_project_dates (GttProjectsTree *gpt,
                                     GtkTreeStore *tree_model, GttProject *prj,
                                     GtkTreeIter *iter)
{
  gtt_projects_tree_set_date_value (gpt, tree_model, iter,
                                    ESTIMATED_START_COLUMN,
                                    gtt_project_get_estimated_start (prj));
  gtt_projects_tree_set_date_value (gpt, tree_model, iter,
                                    ESTIMATED_END_COLUMN,
                                    gtt_project_get_estimated_end (prj));
  gtt_projects_tree_set_date_value (gpt, tree_model, iter, DUE_DATE_COLUMN,
                                    gtt_project_get_due_date (prj));
}

static void
gtt_projects_tree_set_style (GttProjectsTree *gpt, GtkTreeStore *tree_model,
                             GttProject *prj, GtkTreeIter *iter)
{
  GttProjectsTreePrivate *priv = GTT_PROJECTS_TREE_GET_PRIVATE (gpt);
...
static void
gtt_projects_tree_set_project_urgency (GttProjectsTree *gpt,
                                       GtkTreeStore *tree_model,
                                       GttProject *prj, GtkTreeIter *iter)
{
  gchar *value;
...
static void
gtt_projects_tree_set_project_importance (GttProjectsTree *gpt,
                                          GtkTreeStore *tree_model,
                                          GttProject *prj, GtkTreeIter *iter)
{
  gchar *value;
...
static void
gtt_projects_tree_set_project_status (GttProjectsTree *gpt,
                                      GtkTreeStore *tree_model,
                                      GttProject *prj, GtkTreeIter *iter)
{
...
static void
gtt_projects_tree_set_project_data (GttProjectsTree *gpt,
                                    GtkTreeStore *tree_model, GttProject *prj,
                                    GtkTreeIter *iter)
{
  ...
  gtk_tree_store_set (
      tree_model, iter, TITLE_COLUMN, gtt_project_get_title (prj),
      DESCRIPTION_COLUMN, gtt_project_get_desc (prj), TASK_COLUMN,
      gtt_task_get_memo (gtt_project_get_current_task (prj)), SIZING_COLUMN,
      gtt_project_get_sizing (prj), PERCENT_COLUMN,
      gtt_project_get_percent_complete (prj), GTT_PROJECT_COLUMN, prj, -1);
...
static void
gtt_projects_tree_add_project (GttProjectsTree *gpt, GtkTreeStore *tree_model,
                               GttProject *prj, GtkTreeIter *parent,
                               GtkTreePath *path, gboolean recursive)
{
  ...
  row_reference
      = gtk_tree_row_reference_new (GTK_TREE_MODEL (tree_model), path);
  ...
  if (recursive)
    {
      ...
      for (node = gtt_project_get_children (prj); node;
           node = node->next, gtk_tree_path_next (child_path))
        {
          ...
          gtt_projects_tree_add_project (gpt, tree_model, sub_prj, &iter,
                                         child_path, recursive);
        }
...
gtt_projects_tree_populate_tree_store (GttProjectsTree *gpt,
                                       GtkTreeStore *tree_model,
                                       GList *prj_list, gboolean recursive)
{
  GList *node;
      ...
      for (node = prj_list; node; node = node->next, gtk_tree_path_next (path))
        {
          ...
          gtt_projects_tree_add_project (gpt, tree_model, prj, NULL, path,
                                         recursive);
        }
    }
...
void
gtt_projects_tree_populate (GttProjectsTree *proj_tree, GList *plist,
                            gboolean recursive)
{
  GtkTreeStore *tree_model
  ...
  gtt_projects_tree_populate_tree_store (proj_tree, tree_model, plist,
                                         recursive);
  gtk_tree_view_set_model (GTK_TREE_VIEW (proj_tree),
                           GTK_TREE_MODEL (tree_model));
}

static void
gtt_projects_tree_add_column (GttProjectsTree *project_tree,
                              gchar *column_name)
{
      ...
      if (!strcmp (column_name, priv->column_definitions[index].name))
        {
          ...
          if (GTK_IS_CELL_RENDERER_TEXT (c->renderer))
            {
              gtk_tree_view_column_add_attribute (column, c->renderer,
          ...
          if (!strcmp (c->name, "title"))
            {
              gtk_tree_view_set_expander_column (GTK_TREE_VIEW (project_tree),
                                                 column);
            }
...
static void
gtt_projects_tree_update_row_data (GttProjectsTree *gpt, GttProject *prj,
                                   GtkTreeRowReference *row_ref)
{
  ...
  if (path)
    {
      GtkTreeStore *tree_model
          = GTK_TREE_STORE (gtk_tree_view_get_model (GTK_TREE_VIEW (gpt)));
...
void
gtt_projects_tree_set_show_seconds (GttProjectsTree *gpt,
                                    gboolean show_seconds)
{
  GttProjectsTreePrivate *priv = GTT_PROJECTS_TREE_GET_PRIVATE (gpt);
...
void
gtt_projects_tree_set_highlight_active (GttProjectsTree *gpt,
                                        gboolean highlight_active)
{
  GttProjectsTreePrivate *priv = GTT_PROJECTS_TREE_GET_PRIVATE (gpt);
...
gtt_projects_tree_get_selected_project (GttProjectsTree *gpt)
{
  GtkTreeSelection *selection
      = gtk_tree_view_get_selection (GTK_TREE_VIEW (gpt));
  ...
  if (row)
    {
      ...
      for (node = gtt_project_get_children (prj); node; node = node->next)
        {
          GttProject *sub_prj = node->data;
          gtt_projects_tree_remove_project_recursively (gpt, sub_prj, model,
                                                        row_references);
        }
    }
}
...
gtt_projects_tree_remove_project (GttProjectsTree *gpt, GttProject *prj)
{
  ...
  gtt_projects_tree_remove_project_recursively (gpt, prj, model,
                                                priv->row_references);
}
                                               ...
                                               GttProject *parent)
{
  ...
  gtt_projects_tree_set_project_data (gpt, GTK_TREE_STORE (model), prj,
                                      &prj_iter);
  ...
  gtk_tree_path_free (path);
  GList *children;
  for (children = gtt_project_get_children (prj); children;
...
void
gtt_projects_tree_append_project (GttProjectsTree *gpt, GttProject *prj,
                                  GttProject *parent)
{
  GttProjectsTreePrivate *priv = GTT_PROJECTS_TREE_GET_PRIVATE (gpt);
  GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (gpt));
 ...
 */
void
gtt_projects_tree_insert_project_before (GttProjectsTree *gpt, GttProject *prj,
                                         GttProject *sibling)
{
  GttProjectsTreePrivate *priv = GTT_PROJECTS_TREE_GET_PRIVATE (gpt);
  GtkTreeModel *model = gtk_tree_view_get_model (GTK_TREE_VIEW (gpt));
  ...
  if (sibling)
    {
      ...
      gtk_tree_store_insert_before (GTK_TREE_STORE (model), &prj_iter, NULL,
                                    &sib_iter);
      gtk_tree_path_free (path);
    }
  else
                                                ...
                                                gpointer data)
{
  ...
  gtt_projects_tree_set_project_data (gpt, GTK_TREE_STORE (tree_model), prj,
                                      iter);
}
  ...
  if (prj)
    {
      ...
      if (gtk_tree_model_iter_parent (model, &parent_iter, iter))
        {
          gtk_tree_model_get (model, &parent_iter, GTT_PROJECT_COLUMN, &parent,
                              -1);
        }
...
static gboolean
count_rows (GtkTreeModel *model, GtkTreeIter *iter, GtkTreePath *path,
            gpointer data)
{
  int *rows = (int *)data;
  ++*rows;
...
static gboolean
get_expander_state (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                    gpointer data)
{
...
static gboolean
set_expander_state (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                    gpointer data)
{
  ExpanderStateHelper *esh = (ExpanderStateHelper *)data;
...
gtt_projects_tree_get_col_width (GttProjectsTree *gpt, int col)
{
  ...
  GtkTreeViewColumn *column
      = gtk_tree_view_get_column (GTK_TREE_VIEW (gpt), col);
  if (column == NULL)
    {
      return -1;
    }
...
void
gtt_projects_tree_set_sorted_column (GttProjectsTree *gpt,
                                     GtkTreeViewColumn *column)
{
      ...
      if (p->data == column)
        {
          gtk_tree_view_column_set_sort_indicator (
              GTK_TREE_VIEW_COLUMN (p->data), TRUE);
          gtk_tree_view_column_set_sort_order (GTK_TREE_VIEW_COLUMN (p->data),
                                               GTK_SORT_ASCENDING);
        }
      else
        {
          gtk_tree_view_column_set_sort_indicator (
              GTK_TREE_VIEW_COLUMN (p->data), FALSE);
        }
    }
...
static void
project_changed (GttProject *prj, gpointer user_data)
{
