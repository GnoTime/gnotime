...
about_box (GtkWindow *w, gpointer data)
{
  ...
  const gchar *comments
      = _ ("GnoTime is a combination of stop-watch, diary, "
  ...
  gtk_show_about_dialog (
      w, "version", VERSION, "program-name", GTT_APP_TITLE, "authors", authors,
      "documenters", documenters, "website", "http://gnotime.sourceforge.net/",
      "translator-credits", _ ("translator-credits"), "copyright", copyright,
      "comments", comments, "license", license, NULL);
}
...
project_name_desc (GtkDialog *w, gint response_id, GtkEntry **entries)
{
  ...
  gtt_projects_tree_append_project (projects_tree, proj,
                                    gtt_project_get_parent (sib_prj));
...
new_project (GtkWidget *widget, gpointer data)
{
  ...
  g_signal_connect (G_OBJECT (dlg), "destroy", G_CALLBACK (free_data),
                    entries);
...
menu_properties (GtkWidget *w, gpointer data)
{
  ...
  if (prj)
    ...
    {
      GtkWidget *dlg
          = gtk_message_dialog_new (GTK_WINDOW (app_window), GTK_DIALOG_MODAL,
                                    GTK_MESSAGE_INFO, GTK_BUTTONS_OK,
                                    _ ("You must select the project of which "
                                       "you want to edit the properties"));
...
menu_howto_edit_times (GtkWidget *w, gpointer data)
{
  ...
  GtkWidget *mb;
  mb = gtk_message_dialog_new (NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
