 ...
 */
static GtkWidget *
toolbar_append_stock_button (GtkToolbar *toolbar, const gchar *text,
  ...
  if (config_show_toolbar)
    {
      ...
      if (config_show_tb_prop)
        {
          mytbar->prop_w = gtk_toolbar_insert_stock (
              mytbar->tbar, GTK_STOCK_PROPERTIES,
      ...
      if (((config_show_tb_timer) || (config_show_tb_journal)
           || (config_show_tb_calendar) || (config_show_tb_prop))
      ...
      if (config_show_tb_pref)
        {
          mytbar->pref = gtk_toolbar_insert_stock (
              ...
              NULL, (GtkSignalFunc)menu_options, NULL, position++);
        }
      if (config_show_tb_help)
        {
          mytbar->help = gtk_toolbar_insert_stock (
              mytbar->tbar, GTK_STOCK_HELP, _ ("User's Guide and Manual"),
              NULL, (GtkSignalFunc)gtt_help_popup, NULL, position++);
        }
      if (config_show_tb_exit)
        {
          mytbar->exit = gtk_toolbar_insert_stock (
              mytbar->tbar, GTK_STOCK_QUIT, _ ("Quit GnoTime"), NULL,
              (GtkSignalFunc)app_quit, NULL, position++);
        }
    }
